...
/**
 * End-to-end test against a real PostgreSQL instance started by Testcontainers
 * (wired in via {@link TestcontainersConfiguration} and {@code @ServiceConnection}).
 *
 * <p>Creates a vehicle, adds a modification and a dyno result, then asserts the
 * aggregated summary - exercising controller -> service -> repository -> Postgres.
 * Requires Docker; runs under {@code mvn verify} (Failsafe), not {@code mvn test}.
 */
@SpringBootTest
...
class VehicleApiIT {
    ...
    void createVehicleAddModificationAndDynoThenSummarise() throws Exception {
        ...
        String vehicleId = objectMapper.readTree(created.getResponse().getContentAsString())
                .get("id").asText();

        mockMvc.perform(post("/api/v1/vehicles/{id}/modifications", vehicleId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        mockMvc.perform(post("/api/v1/vehicles/{id}/dyno", vehicleId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        mockMvc.perform(get("/api/v1/vehicles/{id}/summary", vehicleId))
