...
public class GlobalExceptionHandler {
  ...
  // Each @ExceptionHandler maps one exception type to a specific HTTP status +
  // JSON body.
  ...
  // Catch-all safety net: anything not handled above becomes a 500 with a generic
  // message (no stack
  // trace leaked).
  @ExceptionHandler(Exception.class)
  public ResponseEntity<ErrorResponse> handleUnexpected(Exception ex) {
    // Log the real cause server-side for debugging, while the client only sees a
    // generic error.
